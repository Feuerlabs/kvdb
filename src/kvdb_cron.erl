...
create_crontab(Db, Tab) ->
    kvdb:add_table(Db, Tab, [{type, {keyed, fifo}},
...
add(Db, Tab, Q, When, Options, M, F, As) when is_list(When); is_integer(When) ->
    ...
    case new_jobs(When, Options, M, F, As) of
	[{NextT,_,_}|_] = Entries ->
	    ?debug("pushing entries ~p~n", [Entries]),
	    [kvdb:push(Db, Tab, Q, Entry) || Entry <- Entries],
...
reschedule_job(#job{next = Last, spec = Spec} = Job,
    ...
    if is_integer(Spec) ->
	    ...
	    ok;
       true ->
	    reschedule_(decr_iter(Job), Last, Tab, Q, Db, Opts, Parent)
    end.
...
do_dispatch_(TS, Tab, Q, Db, Parent) ->
	...
	{ok, active, #q_key{key = TS1} = QK, {_,Opts,Job}} when TS1-TS < 1000 ->
	    ...
	    reschedule_job(Job, Tab, Db, Q, Opts, Parent);
	done ->
	    ok
    end.
...
decr_iter(#job{iterations = 0} = J) ->
    J#job{next = never};
decr_iter(#job{iterations = N} = J) when is_integer(N) ->
    case N-1 of
	N1 when N > 0 ->
	    J#job{iterations = N1};
	0 ->
	    J#job{next = never, iterations = 0}
    end;
decr_iter(J) ->
    J.
...
valid_time(Spec, Now, Job) ->
    ...
    Iter = valid_iterations(Spec, Now),
    Int = valid_interval(Spec, Job),
    Until = valid_until(Spec, Int, TS),
    Job#job{next = TS, iterations = Iter, interval = Int, until = Until}.

valid_iterations(_, #job{iterations = It}) when It=/=undefined ->
    It;
valid_iterations(Spec, #job{iterations = undefined}) ->
    case get_opt([iterations, iter], Spec, 1) of
	any ->
	    any;
	I when is_integer(I), I > 0 ->
	    I;
	Other ->
	    error({bad_iterations, Other})
    end.

valid_interval(_, #job{interval = I}) when I =/= undefined ->
    I;
valid_interval(Spec, #job{interval = undefined}) ->
    case get_opt([interval, int], Spec, once) of
	{1,times} -> once;
	{N,Unit}=I when is_integer(N), N > 0, is_atom(Unit) ->
	    case lists:member(Unit, [ms, seconds, minutes, hours,
				     days, weeks, months, years,
				     last_day_of_month, times]) of
		true ->
		    {N, Unit};
		false ->
...
valid_until(Spec, Int, TS) ->
	...
	undefined ->
	    case proplists:get_value(repeat, Spec, undefined) of
		undefined ->
		    {undefined, undefined};
		1 ->
		    TS;
